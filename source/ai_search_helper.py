...
def bonusStart():
    for i in listdir(LEVEL2_INPUT_PATH):
        file_name = LEVEL2_INPUT_PATH + '/' + i
        bonus_points, matrix = read_file(file_name)
        ROWS = len(matrix)
        COLS = len(matrix[0])

        output_path = LEVEL2_OUTPUT_PATH + '/' + i.split('.')[0]

        if not os.path.exists('output'):
            os.mkdir('output')
        if not os.path.exists(LEVEL2_OUTPUT_PATH):
            os.mkdir(LEVEL2_OUTPUT_PATH)
        if not os.path.exists(output_path):
            os.mkdir(output_path)
        if not os.path.exists(output_path + '/gbfs'):
            os.mkdir(output_path + '/gbfs')
        ...
        draw(WIN, grid, ROWS, width,bonus_points)
        resetTmpImage()
        isPath, length = GBFS_bonus_point(WIN, lambda: draw(WIN, grid, ROWS, width, bonus_points), grid, grid[start[0]][start[1]], grid[end[0]][end[1]], ManhattanDistance,bonus_points)   #having lambda lets you run the function inside the function
        createVideo(output_path + '/gbfs/gbfs_heuristic_1.mp4')
        createText(isPath, length, output_path + '/gbfs/gbfs_heuristic_1.txt')
        ...
        draw(WIN, grid, ROWS, width,bonus_points)
        resetTmpImage()
        isPath, length = GBFS_bonus_point(WIN, lambda: draw(WIN, grid, ROWS, width, bonus_points), grid, grid[start[0]][start[1]], grid[end[0]][end[1]], EuclidDistance,bonus_points)   #having lambda lets you run the function inside the function
        createVideo(output_path + '/gbfs/gbfs_heuristic_2.mp4') 
        createText(isPath, length, output_path + '/gbfs/gbfs_heuristic_2.txt')
        ...
        draw(WIN, grid, ROWS, width,bonus_points)
        resetTmpImage()
        isPath, length = Astar_bonus_point(WIN, lambda: draw(WIN, grid, ROWS, width, bonus_points), grid, grid[start[0]][start[1]], grid[end[0]][end[1]], EuclidDistance,bonus_points)   #having lambda lets you run the function inside the function
        createVideo(output_path + '/astar/astar_heuristic_2.mp4') 
        createText(isPath, length, output_path + '/astar/astar_heuristic_2.txt')
